...
drought_exp <- read_xlsx(
  ...
  # specify sheet I want to read in
  sheet = "First Harvest"
...
drought_exp_clean <- drought_exp |> # INSERT COMMENT HERE
  ...
  # creating new column with full names for water treatment 
...
```{r model-selection}
AICc( # simplest model that gives most information
  model0,
  model1,
  model2,
  model3,
...
```{r model-summary}
summary(model4)
# reference level for water_treatment: Drought stress, does no show up
...
**What is the best model?**
 ...
 The best model that explains total plant math includes water treatment and species (F(7,62 = 27.08, p < 0.001, $\aplha$ = 0.05), but not SLA. 

**How much variation in the response (total mass, in grams) does this model explain?**

This model explains 73% (adjusted R^2^) of hte variation in total mass. 

**How do we interpret the effects of the predictors on the response variable (again, total mass in grams)?**

On average, plant masses in drought stressed conditions wew 0.12 $/pm$ 0.02 g less than plant masses in well watered conditions across speices.

Across water treatments, all speciies have different (and higher) masses than _lotus scoparius_ except _Penstemon centranthifolius_ 
...
model4_predictions <- ggpredict(
  model4, 
  terms = c("species_name", "water_treatment") # listing precitors
  
) |> 
  # rename columns (treating like a dta frame)
